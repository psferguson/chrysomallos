...
def get_stamp_centers(tract, patch, butler, cutout_size_x=600, cutout_size_y=600):
    ...
    try:
        calexp = butler.get("deepCoadd_calexp", dataId={'tract': tract, 
                                                    ...
                                                    'band': 'g',
                                                    'skymap': 'hsc_rings_v1'}, 
                                                    collections=['HSC/madcash/ngc4214_4244', 'HSC/calib/madcash/ngc247'])
    except:
        print(f"Failed to get calexp for tract {tract}, patch {patch} 'g'. Skipping.")
        return pd.DataFrame()
    try:
        calexpi = butler.get("deepCoadd_calexp", dataId={'tract': tract, 
                                                        'patch': patch,
                                                        'band': 'i',
                                                        'skymap': 'hsc_rings_v1'}, 
                                                        collections=['HSC/madcash/ngc4214_4244', 'HSC/calib/madcash/ngc247'])
    except:
        print(f"Failed to get calexp for tract {tract}, patch {patch} 'i'. Skipping.")
        return pd.DataFrame()    
    bbox = calexp.getBBox()
    wcs = calexp.getWcs()
    ...
    injection_bool = np.zeros(len(x_cens), dtype=bool)
